...
module.exports.createOrder = async function(req,res){
    try {
        ...
        if(numberOfOrders<10 || typeof(numberOfOrders) == "undefined") {
            ...
            return res.status(201).send({status:true,message:`Order placed, you are ${orderRemaining} orders behind to become a gold member and enjoing 10% discount`,orderDetails:orderData})
        }
        if(numberOfOrders>=10 && numberOfOrders<20){
            ...
            return res.status(201).send({status:true,message:`Order placed, you are ${orderRemaining} orders behind to become a platinum member and enjoing 20% discount`,orderDetails:orderData})
        }
        if(numberOfOrders>=20){
            ...
            return res.status(201).send({status:true,message:`20% discount creditted to your account`,orderDetails:orderData})
        }
    } catch (error) {
